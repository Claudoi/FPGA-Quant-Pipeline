"""Testbench cocotb de la sonda URAM serializada (fase3-uram, criterios 2-3) —
área uram.

Espejos SEC-URAM-01 (lectura registrada, nunca combinacional; ≤ 1 slot/ciclo)
y SEC-URAM-02 (prefetch del grupo de hash durante ST_BODY). Pinza
ESTRUCTURAL: lee señales internas del book (`/* verilator public */` en
`st`, `pr_phase`, `rd_addr`, `rd_data`, `pr_pending_*`) — el criterio 9 de
fase 3 era solo documentación; aquí el retardo de lectura y la serialización
se verifican por señal, no por auditoría.
...
from test_orderbook import (A, E, S, run_book, _reset)
...
async def drive_sampling(dut, messages, max_cycles=400000, window=200):
    """Conduce mensajes a DW=32 y devuelve (out, trace): out = eventos BBO;
    trace = [(st, pr_phase, pr_pending_old, pr_pending_new, rd_addr, rd_data)]
    por ciclo (muestreo en vivo de las señales internas del book)."""
    await _reset(dut)
...
async def test_sec_uram_01_la_tabla_se_lee_de_forma_registrada_nunca_combinacional(dut):
    ...
    out, trace = await drive_sampling(dut, msgs)

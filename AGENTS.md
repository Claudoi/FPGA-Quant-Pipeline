...
| 3 — DW=32/URAM | **No cerrada**: cadena BBO/depth real verde para ND=5/3. Run Vivado 2026-08-18 (synth+place+route, wrapper `itch_chain_synth.sv`): tabla inferida en **32 URAM288 (66,67 %)**, DRC 0, pero **WNS < 0 y LUT > 95 % en los 5 runs**. Historial: base WNS -10,492 ns / LUT 100,33 % → iter 7 (A/B/C) -7,395 / 96,49 % → iter 8 (decode partido) -4,052 / 95,68 % → iter 9 (guard solo tvalid + find-first precomputado) **-3,527 / 95,80 %** → iter 10 (IOB=TRUE + tready registrado, wrapper) -3,748 / 95,79 %. El retiming del book cerró el cuello lógico (familia `depth_tready`→URAM del run 8 eliminada); el residual es **I/O del wrapper**: los FFs de salida del book tienen fanout interno (retención + guard) y el IOB packing no los mueve; el candidato documentado sin aplicar es el pipeline de salida con retención del lado del pin (addendum iter 10). **El criterio 10 sigue abierto** (WNS < 0, TNS ≠ 0, LUT > 95 %); el historial completo de runs vive en `synth/reports/README.md`. **Pendiente**: rojo→verde de `sim-rtm`/`sim-rtm64`/`sim-lat` (iter 7/8/9 juntas) y gates A/E/B/C en la máquina con cocotb; SEC-URAM-04 enmendado a media ≤ 48 (RTM-LAT-01). REP-02 (tramo A/U real `<=24` stalls) sigue abierto. |
| 4 — CME MDP3 | **Reabierta funcionalmente**: falta cerrar framing `s_axis_tkeep`; schema/version, límite de tamaño y backpressure de salida se tratarán en loops separados. Sin Vivado no se acredita timing. El loop de framing está preparado: addendum en la spec (contrato del puerto `s_axis_tkeep`, orden rojo→verde), escenario M3-FRM-05 en `mdp3.feature`, driver del área mdp3 con `tkeep` MSB-contiguo y `test_m3frm05_tkeep_bytes_validos_y_truncado_por_mascara` (SkipTest declarado mientras el RTL no exponga el puerto; py_compile verde). **Pendiente**: implementar el framing en `rtl/parser/mdp3_parser.sv` y cerrar rojo→verde en la máquina con cocotb. |

...
| 3 — DW=32/URAM | **No cerrada**: cadena BBO/depth real verde para ND=5/3. Run Vivado 2026-08-18 (synth+place+route, wrapper `itch_chain_synth.sv`): tabla inferida en **32 URAM288 (66,67 %)**, DRC 0, pero **WNS = -10,492 ns** (periodo 3,103 ns), TNS = -590.856,875 ns y **LUT al 100,33 %** — el cuello es la generación de BBO/depth (37-41 niveles de lógica), no la URAM ni el parser. El loop del retiming del escaneo de niveles (addendum iter 7: ST_EMIT → etapas A/B/C registradas, LUT ≤ 95 %, WNS ≥ 0) **está implementado y commiteado** (`2fa7250`, RTL + tests RTM-01..04/RTM-LAT-01/RTM-REG-01 + targets `sim-rtm`/`sim-rtm64`; checks estáticos verdes aquí: py_compile, gate F, synth_check 24/24, xvlog 0 errores). **Re-run Vivado 14:11 (mismo tcl, gate abortó como está diseñado)**: WNS = **-7,395 ns** (+3,1 ns), TNS = **-430.582,411 ns** (+160 µs), LUT as Logic **96,49 %** (-3,84 pp), URAM 32/48 igual; peor ruta interna `lv_eq → lv2_mode` con 31 niveles (etapa B del find-first no quedó corta) y peor ruta absoluta = I/O del wrapper (`msg_len_reg → s_axis_tready`, 11 niveles). **El criterio 10 sigue abierto** (WNS < 0, TNS ≠ 0, LUT > 95 %). **Pendiente**: iter 8 con spec (registrar salidas del wrapper y/o partir la etapa B en dos registros), rojo→verde de `sim-rtm`/`sim-rtm64`/`sim-lat` y gates A/E/B/C en la máquina con cocotb. SEC-URAM-04 enmendado a media ≤ 48 (RTM-LAT-01). REP-02 (tramo A/U real `<=24` stalls) sigue abierto. |

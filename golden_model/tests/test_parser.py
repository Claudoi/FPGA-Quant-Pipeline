"""Tests espejo de specs/fase0-golden-model/gherkin/parser.feature.

Oraculo independiente: los payloads sinteticos se construyen aqui con offsets
escritos a mano desde NQTVITCHSpecification.pdf. NADA se importa de
golden_model.itch.messages para fabricar mensajes: si la tabla canonica esta
mal transcrita, estos tests lo gritan.
"""
...
# --- constructores de payload con offsets literales de la spec PDF ---------

def _hdr(mtype: bytes, locate: int, tracking: int, ts: int) -> bytes:
    # offsets 0,1,3,5: tipo(1) locate(2) tracking(2) timestamp(6)
...
def binaryfile(*payloads: bytes) -> io.BytesIO:
    """Stream BinaryFILE: length u16be + payload (framing de la spec BinaryFILE)."""
    return io.BytesIO(b"".join(len(p).to_bytes(2, "big") + p for p in payloads))


# Tabla de longitudes transcrita a mano de NQTVITCHSpecification.pdf
# (oráculo independiente de messages.py: si las dos transcripciones difieren,
# este test lo grita).
KNOWN_LENGTHS = {
...
class TestParser(unittest.TestCase):
    def test_par01_iterar_un_binaryfile_completo_sin_errores(self):
        # un BinaryFILE "completo": un mensaje de cada tipo ITCH 5.0
        ...
        # y con mensajes con campos reales
        stream = binaryfile(p_s(), p_r(), p_h(), p_a(), p_e(), p_c(), p_x(), p_d(), p_u(), p_f())
        msgs = list(iter_messages(stream))
        self.assertEqual(len(msgs), 10)
        self.assertEqual(msgs[-1][0], 9)  # ultimo indice = total - 1

    def test_par02_conteo_por_tipo_de_mensaje_del_dia_real(self):
        # el espejo verifica el comportamiento de conteo; el dia real va al verify-report
    ...
    def test_par03_decodifica_campos_completos_del_subset(self):
        casos = [
            # (payload, tipo, campos esperados tras la cabecera)
            ...
            (p_s(), "S", ("O",)),
            (p_h(), "H", ("AAPL", "T", "", "T1")),  # reserved decodifica a ""
        ]
        ...
        # pin de offset independiente: ref de A vive en los bytes 11..18 del payload
        self.assertEqual(p_a()[11:19], (123456789).to_bytes(8, "big"))
        # R: stock y market category (campos del ejemplo del Gherkin)
    ...
    def test_sec02_longitud_incorrecta_para_el_tipo_es_error_duro(self):
        # mas corta de lo especificado
        payload = p_a()[:-1]  # declara 35, el tipo A exige 36
        stream = io.BytesIO(len(payload).to_bytes(2, "big") + payload)
        with self.assertRaises(BadLengthError):
            list(iter_messages(stream))
        # mas larga de lo especificado (mata al mutante `declared < expected`)
        payload = p_a() + b"\x00"  # declara 37
        stream = io.BytesIO(len(payload).to_bytes(2, "big") + payload)
        with self.assertRaises(BadLengthError):
            list(iter_messages(stream))
